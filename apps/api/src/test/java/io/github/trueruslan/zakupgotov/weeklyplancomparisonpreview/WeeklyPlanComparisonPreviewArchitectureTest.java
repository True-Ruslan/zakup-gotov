...
class WeeklyPlanComparisonPreviewArchitectureTest {

    @Test
    void weeklyPlanComparisonPreviewApplicationBoundaryExists() {
        var classes = productionClasses();

        assertThat(classes.stream()
                        .anyMatch(javaClass -> javaClass.getPackageName()
                                .equals("io.github.trueruslan.zakupgotov.weeklyplancomparisonpreview")))
                .as("M3.3 weeklyplancomparisonpreview production package must exist")
                .isTrue();
    }

    @Test
    void composedBoundaryDoesNotReachIntoPlannerRecipeOrComparisonInternals() {
        var classes = productionClasses();

        noClasses()
                .that().resideInAPackage("..weeklyplancomparisonpreview..")
    ...
    void acceptedApplicationBoundariesRemainIndependentFromComposer() {
        ...
        noClasses()
                .that().resideInAnyPackage("..weeklyplanpreview..", "..preview..")
                .should().dependOnClassesThat().resideInAPackage("..weeklyplancomparisonpreview..")
                .check(classes);
    }
