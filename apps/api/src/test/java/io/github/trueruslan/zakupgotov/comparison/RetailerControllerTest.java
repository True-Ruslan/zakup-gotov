...
class RetailerControllerTest extends PostgresIntegrationSupport {
    ...
    void returnsAllCanonicalRetailersWithProductSafeReadinessState() throws Exception {
        mockMvc.perform(get("/api/v1/retailers"))
                ...
                .andExpect(jsonPath("$.retailers[3].productionAccess").value("BLOCKED"))
                .andExpect(jsonPath("$.retailers[3].comparisonStatus").value("UNAVAILABLE"))
                .andExpect(jsonPath("$.retailers[3].reasons", hasSize(1)))
                .andExpect(jsonPath("$.retailers[3].reasons[0]").value("PRODUCTION_ACCESS_BLOCKED"))
                .andExpect(jsonPath("$.retailers[3].total").doesNotExist())
                .andExpect(jsonPath("$.retailers[3].freshness").doesNotExist())
