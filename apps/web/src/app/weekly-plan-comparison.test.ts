...
type Request = components["schemas"]["WeeklyPlanPantryOptimizationPreviewRequest"];
...
describe("WeeklyPlan Pantry optimization transport", () => {
  it("fails closed without API_BASE_URL and performs no request", async () => {
    delete process.env.API_BASE_URL;
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);

    await expect(createWeeklyPlanOptimizationPreview(request)).resolves.toEqual({ kind: "unavailable" });
    expect(fetchMock).not.toHaveBeenCalled();
  });

  it("uses only the generated M4.4.1 path and returns the typed preview unchanged", async () => {
    process.env.API_BASE_URL = "http://api.test";
    const response: components["schemas"]["WeeklyPlanPantryOptimizationPreview"] = {
    ...
    const fetchMock = vi.fn((input: RequestInfo | URL) => {
      ...
      expect(url).toBe("http://api.test/api/v1/weekly-plan-pantry-optimization-previews");
      expect(url).not.toContain("weekly-plan-pantry-comparison-previews");
    ...
    await expect(createWeeklyPlanOptimizationPreview(request)).resolves.toEqual({
      kind: "ready",
      data: response,
    });
  ...
  it("returns only generated product-safe M4.4.1 validation fields for HTTP 400", async () => {
    process.env.API_BASE_URL = "http://api.test";
    vi.stubGlobal(
      "fetch",
  ...
  it("aborts a hanging backend request within the bounded timeout", async () => {
    ...
    const result = createWeeklyPlanOptimizationPreview(request);
