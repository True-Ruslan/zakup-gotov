...
Both PRs remain diagnostic-only: neither route is accepted as store context, and no offer/availability mapping was enabled. The open question for #169 is now narrower and code-grounded rather than speculative: whether the accepted `v2|v3/stores/{id}/...` resource re-fires after a SPA-navigation reset on a run with no active order. Further ordinary-user-browser runs are needed to confirm.
