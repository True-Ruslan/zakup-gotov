...
A follow-up automated-browser reconnaissance session against `app.chizhik.club` (run by Claude, not an ordinary-user session; see invariant 20) observed a successful `/delivery/api/catalog/v1/categories/inout?store_id=...` request before the session was blocked by the site's own WAF (`403 Forbidden`, consistent with D1's finding that automated Chromium is not the selected acquisition environment). That route shape does not match the accepted `v2|v3/stores/{sap_id}/...` contract and was not previously distinguished from the generic "other version" flag. PR #193 (squash-merged `7f68cb5404df77e33c08f350bdd02f033de45099`) added a dedicated `storeScopedCategoriesInoutSeen` / `store_categories_inout` diagnostic flag for this shape, verified by unit and Playwright E2E coverage before merge. This is diagnostic-only: `categories/inout?store_id=` is not accepted as a resolvable store context, and no offer/availability mapping was enabled.

Two ordinary-user-browser #169 canary runs then supplied the first real evidence:

1. `store_v2_v3=SEEN` together with `store_categories_inout=SEEN`, yet `status=MISSING_CONTEXT` persisted. This is not a diagnostics defect: `CHIZHIK_D2_DIAG` scans the full page Resource Timing history at click time, while `resolveChizhikEvidencedStoreId` only trusts the content script's own incrementally-maintained `observedResourceUrls` set, which is deliberately cleared on every SPA-navigation boundary (invariant 19) and must be reseeded by a fresh accepted-shape resource afterward. The most likely explanation is that the accepted `v2|v3/stores/{id}/...` resource fired once during initial store selection, a subsequent same-document navigation to search results reset the live context set, and the site did not re-issue that exact resource afterward — only `categories/inout` fired post-reset, which the resolver does not accept.
2. A second run returned `delivery_catalog=NOT_SEEN` with every catalog-scoped flag `NOT_SEEN`, leaving `delivery_api=SEEN` unexplained. Sanitized path-shape evidence (URLs only, no headers/geolocation/device-id/order-id) showed the page had instead called `/delivery/api/orders/v{N}/...` endpoints (active-order status, order detail, an orders-scoped nearby-store lookup by geolocation) — an entirely different route family outside every existing flag's scope, most likely because an order was in progress on that run. PR #195 (squash-merged `93adee242a5dc23158a5d96837c5a7269ec2b15e`) added `deliveryOrdersSeen` / `delivery_orders` for this shape, independent of the catalog-scoped early return, verified by unit and Playwright E2E coverage before merge.

Both PRs remain diagnostic-only: neither route is accepted as store context, and no offer/availability mapping was enabled. A third ordinary-user-browser run, deliberately controlled (no active order, hard cache-clearing reload before invocation), reproduced the same outcome: `store_v2_v3=SEEN` yet `status=MISSING_CONTEXT`, and `delivery_orders=SEEN` even with no order in progress — disproving the earlier speculation that `delivery_orders` correlates with an active order; it evidently fires during ordinary navigation regardless. This closes the "was it an active order?" branch of the open question and narrows it further: `content.ts`'s `handleSameDocumentNavigation` clears `observedResourceUrls` and sets `awaitingFreshContext=true` on every same-document navigation, and only a resource matching the exact accepted store-context shape observed *after* that reset clears `awaitingFreshContext` again (`content.ts` `rememberAllowedResource`). Two out of two controlled runs now show the accepted resource firing once (at initial store selection) and not re-firing after navigating to search results, so `awaitingFreshContext` stays set and context resolution fails closed for what is, from the user's perspective, an entirely ordinary search flow.
