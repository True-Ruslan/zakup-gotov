...
### Chizhik D2 missing-context diagnostics — IMPLEMENTED / MERGED; TWO LIVE RUNS ANALYZED

A real ordinary-browser #169 run reproduced `MISSING_CONTEXT` even while the official search UI showed results, disproving the assumption that normal search interaction reliably exposes an accepted store-scoped resource. PR #192 (`940b242`) added privacy-safe route-family diagnostics (fixed `SEEN`/`NOT_SEEN` booleans, no raw URL/store ID) instead of guessing a new accepted route. PR #193 (`7f68cb5`) extended those diagnostics with one more flag for a `/delivery/api/catalog/v1/categories/inout?store_id=...` route shape observed during automated reconnaissance — a shape that does not match the accepted `v2|v3/stores/{sap_id}/...` contract.

Three subsequent ordinary-user-browser runs used these diagnostics against the live site. Run 1 showed `store_v2_v3=SEEN` and `store_categories_inout=SEEN` together with `status=MISSING_CONTEXT`: the accepted-shape resource is only visible in the page's full Resource Timing history, not necessarily in the content script's live, SPA-navigation-reset context set that the resolver actually trusts (invariant 19) — most likely the accepted resource fired once before a search navigation reset it, and did not re-fire after. Run 2 showed `delivery_catalog=NOT_SEEN` with every catalog-scoped flag `NOT_SEEN`, which no existing flag could explain; sanitized path-shape evidence showed the page had instead called `/delivery/api/orders/...` endpoints (active-order status/detail, an orders-scoped nearby-store lookup), most likely because an order was in progress. PR #195 (`93adee2`) added a `deliveryOrdersSeen` / `delivery_orders` flag for this shape. Run 3 deliberately controlled for both open variables — no active order, and a hard cache-clearing reload immediately before invocation — and still reproduced `store_v2_v3=SEEN` with `status=MISSING_CONTEXT`, while `delivery_orders=SEEN` fired anyway, disproving the active-order correlation from Run 2. All three PRs remain diagnostic-only.

The open question is now code-grounded rather than speculative: `apps/retailer-bridge/src/content.ts`'s `handleSameDocumentNavigation` clears the live `observedResourceUrls` set and sets `awaitingFreshContext=true` on *every* same-document (SPA) navigation; only a resource matching the exact accepted `v2|v3/stores/{id}/...` shape observed *after* that reset clears `awaitingFreshContext` again. Two of two controlled runs show that resource firing once at initial store selection and never re-firing after navigating to search results, so ordinary search browsing systematically fails context resolution under the current design — not because context is genuinely absent, but because the accepted resource simply isn't re-issued by the live site after the first store selection. The next step is a product/security decision, not another diagnostic: whether to retain the already-evidenced store context across a same-document navigation unless a *conflicting* (different, valid) store context is later observed, instead of unconditionally discarding it and waiting for a re-fire. That would need to go through the same RED-test-first, security-reviewed process as the original #173/#174 store-context rule before any implementation.
