...
Two subsequent ordinary-user-browser runs used these diagnostics against the live site. Run 1 showed `store_v2_v3=SEEN` and `store_categories_inout=SEEN` together with `status=MISSING_CONTEXT`: the accepted-shape resource is only visible in the page's full Resource Timing history, not necessarily in the content script's live, SPA-navigation-reset context set that the resolver actually trusts (invariant 19) — most likely the accepted resource fired once before a search navigation reset it, and did not re-fire after. Run 2 showed `delivery_catalog=NOT_SEEN` with every catalog-scoped flag `NOT_SEEN`, which no existing flag could explain; sanitized path-shape evidence showed the page had instead called `/delivery/api/orders/...` endpoints (active-order status/detail, an orders-scoped nearby-store lookup), most likely because an order was in progress. PR #195 (`93adee2`) added a `deliveryOrdersSeen` / `delivery_orders` flag for this shape. All three PRs remain diagnostic-only. The next step on #169 is an ordinary-user-browser run with no active order, right after a fresh (non-SPA) reload of search results, to test whether the accepted `v2|v3/stores/{id}/...` resource re-fires and resolves.
