...
- Two ordinary-user-browser #169 canary runs used the #192/#193 diagnostics against the live site: one showed `store_v2_v3` and `store_categories_inout` both `SEEN` together with `MISSING_CONTEXT`, most likely because the accepted resource only fired before a SPA-navigation reset and was not re-issued after; the other showed every catalog-scoped flag `NOT_SEEN`, which sanitized path-shape evidence traced to a different `/delivery/api/orders/...` route family (active-order status/detail, an orders-scoped nearby-store lookup), unexplained by any existing flag. PR #195 adds `deliveryOrdersSeen` / `delivery_orders` for that shape. Diagnostic-only: this route is not accepted as store context and no offer mapping is enabled.
